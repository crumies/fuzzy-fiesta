...
struct RPMArc: View {
    ...
    // Starts early enough so redline is visible on real ground speed, not only wheel-in-air max.
    var redlineStartRPM: Double {
        switch mode {
        case .eco: return 1500
        case .xc: return 2300
        case .sports: return 3300
        case .reverse: return 60
        case .park: return 450
        }
    }

    // Final red/black color is reached BEFORE max RPM.
    // That way the last 800-900 rpm is already full redline.
    var fullRedlineRPM: Double {
        switch mode {
        case .eco: return max(redlineStartRPM + 1, modeLimitRPM - 850)
        case .xc: return max(redlineStartRPM + 1, modeLimitRPM - 900)
        case .sports: return max(redlineStartRPM + 1, modeLimitRPM - 900)
        case .reverse: return max(redlineStartRPM + 1, modeLimitRPM - 120)
        case .park: return max(redlineStartRPM + 1, modeLimitRPM - 250)
        }
    }

    var currentRPM: Double {
        min(max(Double(rpm), 0), displayMaxRPM)
    }

    var progress: Double {
        min(max(currentRPM / displayMaxRPM, 0), 0.985)
    }
    ...
    var fullRedlineProgress: Double {
        min(max(fullRedlineRPM / displayMaxRPM, 0), 0.985)
    }
    ...
    var redlineVisualProgress: Double {
    ...
    var activeRedlineColors: [Color] {
        if redlineVisualProgress < 0.35 {
            return [baseColor, baseColor, midColor.opacity(0.7)]
        } else if redlineVisualProgress < 0.70 {
            return [baseColor, midColor, .orange, .red.opacity(0.9)]
        } else {
    ...
    var body: some View {
        ZStack {
            ...
            // Normal base section before redline starts.
            Circle()
            ...
            if progress > redlineStartProgress {
                // Redline gradient section. Reverse now uses the SAME direction as all modes:
                // base color at redline start, final red at the RPM limit side.
                ...
                // After fullRedlineRPM, stay final color until max/headroom.
